...
Created based on [google code labs](https://codelabs.developers.google.com/codelabs/cloud-app-engine-springboot/index.html#0)
 ...
 3. Use **Jib** to create the container image and push it to the Container Registry: 
    `./mvnw -DskipTests com.google.cloud.tools:jib-maven-plugin:build -Dimage=gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:v1`
 4. Test the image locally with the following command which will run a Docker container 
    ...
    `docker run -ti --rm -p 8080:8080 gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:v1`
 5. Deploy your application to Kubernetes `kubectl run employee-backend-server --image=gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:v1 --port=8080`
 6. `kubectl get deployments`
 7. `kubectl get pods`
 8. `kubectl expose deployment employee-backend-server --type=LoadBalancer`
### Package the Java application as a Docker container [Option 2]
 ...
 2. `docker image build -t employee-backend:0.1.0 .`
 3. `docker tag employee-backend:0.1.0 gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:0.1.0`
 4. `docker images`
 5. `docker push gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:0.1.0`
 6. `docker stop $(docker ps -q)`
 7. `docker rm $(docker ps -aq)`
 8. `gcloud config set compute/zone us-central1-a`
 9. `gcloud container clusters get-credentials spinnaker-tutorial`
 10. `kubectl run employee-backend-server --image=gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:0.1.0 --port 8080`
 11. `kubectl expose deployment employee-backend-server --type="LoadBalancer"`
 #### Update the image [Option 2]
 1. `kubectl set image deployment employee-backend-server employee-backend-server=gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:0.1.0`
 2. `kubectl rollout status deployment employee-backend-server`
 3. `kubectl get deployments`
 #### [Optional] To Test  after removing local Child images
 1. `docker stop $(docker ps -q)`
 2. `docker rm $(docker ps -aq)`
 3. `docker rmi employee-backend:0.1.0 gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend employee-backend:0.1.0`
...
##### [Optional] Now we have pseudo env.. Test docker
 7. `docker pull gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:0.1.0`
 8. `docker run -p 8080:8080 -d gcr.io/$GOOGLE_CLOUD_PROJECT/employee-backend:0.1.0`
 ...
 7. `docker image ls`
 8. `docker run -p 3000:80 --rm employee-frontend:0.1` OR `docker run -d  -p 8080:8080 employee-frontend:0.1`
 9. `docker ps`
 10. `docker history employee-frontend:0.1`
 11. `docker stop [container_id]`
 12. `docker tag employee-frontend:0.1 gcr.io/$GOOGLE_CLOUD_PROJECT/employee-frontend:0.1`
 13. `docker images`
 14. `docker push gcr.io/$GOOGLE_CLOUD_PROJECT/employee-frontend:0.1`
 15. `docker stop $(docker ps -q)`
 16. `docker rm $(docker ps -aq)`
 17. `kubectl run employee-frontend-server --image=gcr.io/$GOOGLE_CLOUD_PROJECT/employee-frontend:0.1 --port 80`
 18. `kubectl expose deployment employee-frontend-server --type="LoadBalancer"`
 ### Update the image
 19. `kubectl set image deployment employee-frontend-server employee-frontend-server=gcr.io/$GOOGLE_CLOUD_PROJECT/employee-frontend:0.1`
 20. `kubectl rollout status deployment employee-frontend-server`
